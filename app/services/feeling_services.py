...
logger = logging.getLogger("Felling")
logger.setLevel(logging.DEBUG)
ch = logging.StreamHandler()
ch.setLevel(logging.DEBUG)
fh = logging.FileHandler(os.path.join(log_dir, "Felling.log"), encoding="utf-8")
fh.setLevel(logging.DEBUG)
formatter = logging.Formatter('%(asctime)s | Felling | %(levelname)s ‖ %(message)s', "%Y-%m-%d %H:%M:%S")
...
def requestsFellingllm(messages):
    # 获取当前使用的模型配置
    felling_config = get_llm_config("llm.felling")
    provider = felling_config.get("provider", "local")
    ...
    age = str(os.getenv('age'))
    personality_core = os.getenv('personality_core')
    now = datetime.datetime.now()
    time_str = now.strftime("%Y-%m-%d %H:%M:%S")
    system_prompt = f'''
    你是一个{age}岁的人，核心特征是{personality_core}。
    你正在与用户聊天，当前的消息是{messages}，请你输出一份当前的情感，不超过20字（如：喜悦，开心、兴奋中夹杂着一丝期待）
    仅输出情感内容，不输出多余内容
    '''
    ...
    payload = {
        "model": get_llm_config_value("llm.felling", "model", "unknown/model"),
        "messages": [
            {
                "role": "system",
        ...
        "enable_thinking": get_llm_config_value("llm.felling", "enable_thinking", None),
        "max_tokens": get_llm_config_value("llm.felling", "max_token", 512),
        "temperature": get_llm_config_value("llm.felling", "temp", 0.7)
    }
